...
1. ~~Watch SAM.gov validation emails~~ ✅ **ACTIVE 2026-07-17.** Registration complete under the LLC; UEI `XF73FG8CVMX1` is citable on bids. ~~record the CAGE code~~ ✅ **CAGE `22NH3` recorded 2026-08-03.** Federal identity is now complete: **UEI `XF73FG8CVMX1` + CAGE `22NH3`**. Pipeline unblocked 2026-07-17 (JBSA row + recurring federal channels). **⏰ Next: SAM renewal due 2027-07-07** (annual; lapsing makes the entity ineligible for award). **Calendar reminder set 2026-08-03** for **2027-05-08** (expiry − 60d), Google Calendar event `vkkkblvrbh5otofk290r5mubps`. Deliberately **not** auto-repeating — the expiry moves to whenever you actually renew, so set the next one after renewing and update this row.
