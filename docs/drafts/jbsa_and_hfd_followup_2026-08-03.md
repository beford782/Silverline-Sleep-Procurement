# DRAFT — JBSA RFQ timeline + Houston HFD award follow-up (review before sending)

> **STATUS: DRAFTS ONLY — DO NOT SEND.** Operator (Blake) reviews, fills `[brackets]`, and sends **from
> silverlinesleep** (never Gmail).
>
> **RULE CHANGE since the 2026-06-30 drafts — read this first.** SAM.gov registration went **ACTIVE
> 2026-07-17** under **Continental Silverline Products, LLC**, so **UEI `XF73FG8CVMX1` is now citable on
> federal work.** The old "no UEI / don't imply federal registration" rule is **retired** — for #1 (JBSA)
> citing it is the entire point of the email.
>
> **Still hard rules:**
> - **Do NOT cite a CAGE code.** It was assigned during activation but is still unrecorded in
>   `docs/active_registrations.md:19` (`[record code here]`). Pull it from the activation notice in the
>   silverlinesleep inbox and add it to the ledger **before** sending #1 — a CO may ask for it.
> - **Do NOT imply we hold a contract or submitted a bid on JBSA.** We answered a *Sources Sought
>   questionnaire* (market research) on 2026-07-02. No solicitation has posted.
> - Houston (#2) is **municipal** — UEI/SAM are irrelevant there; don't raise them.

**Signature block (paste on each; operator confirms phone):**
```
[Name], Continental Silverline Products, LLC
Texas-based institutional mattress manufacturer · NAICS 337910
[phone] · beford@silverlinesleep.com
```

---

## 1. JBSA Lackland — Tech Training Dorm Mattresses (RFQ timeline)  (READY; send after CAGE is recorded)

- **To:** Tristan Torres (CS, A1C) — `tristan.torres@us.af.mil`
- **Cc:** N. Marie Phillips (Contracting Officer) — `norena.phillips@us.af.mil`
- **Ref:** Sources Sought **F2M3CH6099A001** — JBSA Tech Training Dorm Mattresses, Bldg 6578
- **Our history:** questionnaire submitted **2026-07-02** (answered as small-business **manufacturer**,
  Open Market). Requirement as described: **1,258 Twin XL** blue nylon innerspring, inverted sealed seam,
  bed-bug-proof, 16 CFR 1633, plus delivery/install and one-for-one removal & disposal. NAICS 337910,
  PSC 7210, small-business FFP.
- **Status check before sending:** as of the 2026-07-23 inspection, no follow-on RFQ had posted. Re-check
  SAM for F2M3CH6099A001 first — if the RFQ **has** posted, don't send this; switch to bidding it.
- **Why now:** we responded to market research a month ago and our eligibility position has materially
  changed since (SAM Active). That is a legitimate, non-pestering reason to make contact.

**Subject:** Continental Silverline Products, LLC — F2M3CH6099A001 (JBSA Dorm Mattresses) follow-up

> Ms. Phillips / A1C Torres,
>
> Continental Silverline Products, LLC responded to the Sources Sought questionnaire for
> **F2M3CH6099A001**, JBSA Tech Training Dorm Mattresses (Bldg 6578), on 2 July 2026. I'm writing with a
> brief status update and one question.
>
> Since our response, our SAM.gov registration has been completed and is **Active**. Our **UEI is
> XF73FG8CVMX1**, registered as Continental Silverline Products, LLC, a small business under **NAICS
> 337910**. Any eligibility qualification in our earlier submission no longer applies — we are fully
> registered and able to receive a federal award.
>
> We remain a strong fit for this requirement. We are a Texas mattress manufacturer located roughly
> [drive time] from Lackland, we build institutional dormitory innerspring mattresses to the construction
> described in the questionnaire, and we certify finished mattresses to **16 CFR Part 1633**. Manufacturing
> in-region also means we can support the delivery, installation, and one-for-one removal and disposal
> scope without subcontracting the logistics out of state.
>
> My question: **can you share the anticipated timeline for the resulting solicitation** — roughly when the
> RFQ is expected to post, and whether the acquisition strategy is still tracking as a small-business
> set-aside? If there is a distribution or interested-vendors list for this requirement, we would like to
> be included.
>
> Happy to provide a capability statement, product specifications, or samples if that would help your
> market research.
>
> Respectfully,
> [signature block]

**Operator checklist**
- [ ] Record the CAGE code in `docs/active_registrations.md` first.
- [ ] Re-check SAM: has the RFQ posted? If yes → bid it instead of sending this.
- [ ] Fill `[drive time]` (Houston→Lackland is ~3 hrs; confirm which plant serves it).
- [ ] Confirm UEI reads exactly `XF73FG8CVMX1`.
- [ ] Send from silverlinesleep. Log the send date in the pipeline row.

---

## 2. City of Houston Fire Department — INF-2026-0194 (award result + next run)  (READY; send now)

- **To:** Angela Weatherspoon, City of Houston — `Angela.Weatherspoon@houstontx.gov`
- **Ref:** **INF-2026-0194**, "Mattresses and Box Spring Bedding (Twin XL)" — Houston Fire Department
- **Our history:** CSP **submitted a bid**, confirmed by operator 2026-07-02 (BeaconBid response
  5/20/2026, confirmation **RUASV**). Solicitation **closed 5/27/2026**; award result not public as of
  2026-08-03 — **68 days** with no result.
- **Requirement:** 80 ea Twin XL (38×80×11.5) + 80 box springs, 16 CFR 1633, delivery to HFD,
  2707 Dalton St, Houston 77017.
- **Recurring:** roughly **2×/year** — prior runs **INF-2025-0116** (buyer Ronald LaBorde) and
  **INF-2025-0447**. The next-run question matters as much as the award answer.
- **No UEI/SAM content** — municipal buy.

**Subject:** INF-2026-0194 (HFD Mattresses & Box Springs) — award status and next solicitation

> Ms. Weatherspoon,
>
> Continental Silverline Products, LLC submitted a response to **INF-2026-0194**, Mattresses and Box Spring
> Bedding for the Houston Fire Department, ahead of the 27 May 2026 closing (BeaconBid confirmation
> **RUASV**).
>
> Could you let me know **whether this solicitation has been awarded**, and if so, to whom and at what
> pricing once that information is publicly releasable? If it has not yet been awarded, I'd appreciate
> knowing where it stands.
>
> Separately — we understand HFD runs this requirement roughly twice a year (we also saw INF-2025-0116 and
> INF-2025-0447). **When is the next solicitation expected to post,** and is there a bidders or
> notification list we should be on so we receive it directly?
>
> For context: we are a Houston-based institutional mattress manufacturer. We build Twin XL innerspring
> mattresses and box springs to the sizes and 16 CFR 1633 flammability requirement in this solicitation,
> and we deliver locally — 2707 Dalton is a short run for us. If there is ever a need for a partial or
> expedited delivery between scheduled buys, we can usually support that.
>
> Thank you,
> [signature block]

**Operator checklist**
- [ ] Confirm the BeaconBid confirmation code `RUASV` before citing it.
- [ ] If the award already went to someone else, still send — the next-run and bidders-list asks are the
      real value, and the recurring cycle is the durable revenue.
- [ ] Send from silverlinesleep. Record the reply (award result + next-run date) on the lead row; if a
      next-run date comes back, it becomes a dated re-bid prep entry.

---

*Prepared 2026-08-03. Sources: `bids/active/_pipeline.csv` (JBSA F2M3CH6099A001),
`leads/review/_lead_radar.csv` (`city-of-houston-beaconbid-...-inf-2026-0194`),
`docs/drafts/jbsa_sources_sought_response_F2M3CH6099A001.md` (contacts + questionnaire scope),
`docs/drafts/recurring_buyer_outreach_2026-06-30.md` #3a (Houston contact research),
`docs/active_registrations.md` (SAM Active 2026-07-17, UEI, open CAGE item).*
