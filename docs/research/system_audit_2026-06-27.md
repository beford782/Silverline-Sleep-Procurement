...
The system is structurally sound and the core discipline (empty active pipeline, human triage, no-bid memos) is being held — but it holds by *convention, not code*, and its dominant failure mode is **silent misses**, the exact class that already cost the ~90-fit Bernalillo correctional RFB. Three systemic weaknesses recur: (1) scheduled ingest jobs swallow RSS/SAM fetch failures and report green, with zero failure alerting; (2) the relevance engine both under-recalls (SAM sweep is a single `title="mattress"` substring) and mis-signals premium fits (anti-ligature penalized, `aviation`/disposal hard-rejects fire before STRONG `mattress`); and (3) governance state (registry status, gate columns, ledger-vs-Radar) is unenforced, so drift is undetectable. One **critical** item gates all federal revenue: SAM.gov is not Active and, as of the later 2026-06-28 verification, the WIP registration displays the wrong L.P. legal name, so EFT/banking must stay paused. The three highest-leverage actions: **(a)** add an `if: failure()` alert + reject-log to every scheduled workflow so misses become visible; **(b)** resolve SAM through FSD/entity validation before any banking or UEI use; **(c)** broaden SAM recall (NAICS 337910 + title sweeps) and fix the relevance precedence/anti-ligature inversions.
...
>
> **Status update (2026-06-28):** The **NM Euna/Bonfire registration is now COMPLETE and verified correct as LLC** — no L.P. display was found. The SAM finding below is also superseded: SAM is blocked by the wrong-named Work-In-Progress record under UEI `XF73FG8CVMX1`; do **not** finish banking or cite that UEI until FSD/entity validation resolves correction vs. fresh LLC UEI.

## Defect list (prioritized)

| Issue | Dimension | Location | Fix |
|---|---|---|---|
| **SAM.gov not Active — wrong-named WIP blocks ALL federal awards** | runbooks-backlog | `docs/active_registrations.md:19,82`; `docs/sam_uei_unblock_runbook.md:17` | Do **not** complete Financial/EFT. Use FSD/entity validation to determine whether UEI `XF73FG8CVMX1` can be corrected to the LLC or must be superseded by a fresh LLC UEI/CAGE. |
...
| SAM runbook had stale LLC/L.P. guidance | runbooks-backlog | `docs/sam_uei_unblock_runbook.md` | Superseded by the 2026-06-28 SAM wrong-name stop block and FSD/entity-validation path. |
...
| **DONE/SUPERSEDED: One canonical `docs/company_identity.md` + corrected SAM wrong-name guidance** | high | low | runbooks-backlog | Create identity card (legal name, TX Limited Liability Company, Certificate of Formation, UEI XF73FG8CVMX1, NAICS 337910/337127, PSC 7210/7105); edit `sam_uei_unblock_runbook.md:41,49,60`; link from all runbooks. |
