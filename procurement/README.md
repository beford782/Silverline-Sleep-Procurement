...
- **Source registry** - `sources/procurement_sources.json`, a
  ...
  intake methods we use against each. SAM.gov is ingested by API, and
  mapped operator-downloaded CSV exports such as ESBD can be imported
  with `tools/ingest_portal_csv.py`.
- **Fit score / risk level** - keyword-driven scoring applied by
  `tools/pipeline.py score`. Positive keywords (mattress, dormitory,
  correctional, cot, shelter, etc.) add to the score; caution keywords
  (anti-ligature, removal, installation, nationwide, multi-year, etc.)
  subtract. Strong-caution terms (`anti-ligature`, `liquidated
  damages`, `nationwide`) force `risk_level=high` regardless. Score
  thresholds: `<25` high, `25-74` medium, `>=75` low. `fit_score` is
  numeric (`0` to `100`); `risk_level` is one of `low`, `medium`, or
  `high`.

## Operational workflow at a glance

1. **Intake** - `tools/pipeline.py add ...` to register a new
   opportunity, `tools/ingest_sam.py` for SAM.gov, or
   `tools/ingest_portal_csv.py` for mapped portal CSV exports such as
   ESBD.
