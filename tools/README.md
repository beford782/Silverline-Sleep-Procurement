...
| `ingest_email.py` | yes | Ingest portal commodity/NIGP email alerts from a Gmail mailbox into the active pipeline. Stdlib HTTPS via `urllib.request` against the Gmail REST API (OAuth refresh-token flow). Requires `GMAIL_CLIENT_ID`/`GMAIL_CLIENT_SECRET`/`GMAIL_REFRESH_TOKEN` (or `--fixture` for offline use). Generic title/link/due-date parser; dedupes by `opportunity_id`. See `docs/email_ingest_setup.md`. |
